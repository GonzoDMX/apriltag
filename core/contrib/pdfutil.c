...
int pdf_objects_equal(const pdf_object_t* a,
    ...
    switch (a->type) {
        ...
        case PDF_OBJECT_STREAM: {
            if (!pdf_objects_equal(a->stream.dict, b->stream.dict)) {
                return 0;
...
pdf_dict_entry_t pdf_dict_get(pdf_object_t* o, int idx) {
    ...
    assert(idx >= 0 && (size_t)idx < zarray_size(o->dict));
...
int pdf_dict_lookup(pdf_object_t* o, size_t length, const char* data) {
    ...
    for (size_t i=0; i < zarray_size(o->dict); ++i) {
...
void pdf_stream_append(pdf_object_t* o, size_t len, const char* data) {
    assert(o->type == PDF_OBJECT_STREAM);
    int oldlen = (int)zarray_size(o->stream.content);
...
void pdf_object_destroy(pdf_object_t* o) {
  ...
  case PDF_OBJECT_NAME:
    ...
    break;
  case PDF_OBJECT_ARRAY:
    for (size_t i=0; i < zarray_size(o->array); ++i) {
      pdf_object_t* oi;
      zarray_get(o->array, i, &oi);
...
int pdf_lookup_toplevel(pdf_t* pdf, pdf_refspec_t refspec) {
    ...
    for (size_t i=0; i < zarray_size(pdf->toplevel_objects); ++i) {
        pdf_toplevel_object_t tlo;
...
void pdf_write(pdf_writer_t* w, size_t len, const char* data) {
    int status = (int)w->callback(len, data, w->userdata);
    if (status != 0) {
        // If an error occurs, perform a long jump to the saved point in 'env'
        // This should be set up with 'setjmp' earlier in the code.
        // Jump to error handling with the error status
        longjmp(w->env, status);
    }
    // Update the number of bytes written after successful writing
    w->bytes_written += len;
}
...
void pdf_write_object(pdf_writer_t* w, pdf_object_t* o) {
    ...
    switch (o->type) {
        case PDF_OBJECT_BOOLEAN:
            ...
            break;
        case PDF_OBJECT_INTEGER: 
            snprintf(buf, 1024, "%d", o->integer);
            // TODO: check return value
            pdf_write_str(w, buf);
            break;
        case PDF_OBJECT_REAL:
            if (o->real == floor(o->real)) {
                snprintf(buf, 1024, "%.0f", o->real);
                // TODO: check return value
            } else {
                snprintf(buf, 1024, "%f", o->real);
                // TODO: check return value
            }
            pdf_write_str(w, buf);
            break;
        ...
        case PDF_OBJECT_DICT:
            ...
            break;
        case PDF_OBJECT_STREAM: {
            int len = (int)zarray_size(o->stream.content);
            if (!o->stream.dict) {
        ...
        case PDF_OBJECT_REFERENCE:
            snprintf(buf, 1024, "%d %d R", o->refspec.id, o->refspec.revision);
            // TODO: check return value
            pdf_write_str(w, buf);
            break;
        default:
...
void pdf_cur_page_newline(pdf_t* pdf) {
    ...
    if (sc->size && !isspace(sc->data[sc->size-1])) {
...
void pdf_image_gray(pdf_t* pdf,
    ...
    pdf_object_t* resources = pdf_dict_lookup_str_create(
                                pdf->cur_page, "Resources", pdf_dict_create);

    pdf_object_t* procset = pdf_dict_lookup_str_create(
                                resources, "ProcSet", pdf_array_create);

    pdf_array_append_unique(procset, pdf_name_create_str("PDF"));
    pdf_array_append_unique(procset, pdf_name_create_str("ImageB"));

    pdf_object_t* xdict = pdf_dict_lookup_str_create(
                                resources, "XObject", pdf_dict_create);

    char oname[1024];
    snprintf(oname, 1024, "Im%ld", zarray_size(xdict->dict)+1);
...
void pdf_path_draw(pdf_t* pdf,
    ...
    for (size_t i=0; i < zarray_size(path); ++i) {
        ...
        default:
            fprintf(stderr,
                    "warning pdf path element %ld has invalid type %d\n",
                    i, (int)element.type);
            break;
        }
...
void pdf_gstate_pop(pdf_t* pdf) {
    ...
    if (zarray_size(pdf->gstate) <= 1) {
        ...
        zarray_truncate(pdf->gstate, zarray_size(pdf->gstate)-1);
    }
